...
Non, le code n'est pas structuré en couche car les différents couches sont tous dans le même repertoire.
Les couches sont emmelées et, si l'application continue de se développer, il va être difficile de les séparer par la suite.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier (dont la règle de gestion RG 1) se trouve dans dans la classe TodoListController.

*Cette règle est-elle facilement testable par un test unitaire ?*
Oui, cette méthode est testable par un test unitaire car elle n'a qu'une seule fonction et qu'il est facile d'isoler son comportement.
...
docker run : permet de créer et lancer un conteneur docker
-it : Lance la conteneur en mode intéractif
-e : Configure les variables d'environnements, ici POSTGRES_PASSWORD qui est initialisée avec la valeur password
-p : Permet de lier un port du conteneur sur un port de la machine
postgres : Défini l'image du conteneur lancé

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
Le `todoItemRepository` esy `null` car il n'a pas été injecté dans le controller. La meilleure de l'injecter, est en ajoutant l'annotation `@Inject` au dessus du constructeur.
...
L'architecture hexagonale sert à séparer la logique métier de l'infrastructure. Le domaine métier doit pouvoir fonctionner seul, sans dépendre de l'infrastructure technique.
Cette architecture s'appuie sur les ports (in et out) et adapters avec le domaine au centre.
...
Pour les adapteurs il faudra écrire des TIs (Tests d'ingrétation)
